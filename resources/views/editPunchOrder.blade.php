...
<body>
    ...
    <section class="punch-order-form-page">
        <h1 class="page-title">Edit Punch Order #{{ $punchOrder->id }}</h1>
        ...
        <form method="POST" action="/admin/stock/punch-orders/edit/{{ $punchOrder->id }}" id="punchOrderForm">
            @csrf

            <div class="form-group">
                <label for="note">Note (optional)</label>
                <input type="text" id="note" name="note" value="{{ old('note', $punchOrder->note) }}">
            </div>
    ...
    @php
        $initialRows = $punchOrder->items->map(function ($item) {
